...
pub(crate) fn render_line(c: &LineChart) -> Result<String, ChartError> {
    let mut buf = String::new();
    {
        ...
        let (x_min, x_max) = c.x_range.unwrap_or_else(|| auto_x_range(&c.series));
        let (y_min, y_max) = c.y_range.unwrap_or_else(|| auto_y_range(&c.series));
...
fn auto_x_range(series: &[LineSeries]) -> (f64, f64) {
    let mut min = f64::MAX;
    let mut max = f64::MIN;
    for s in series {
        for (x, _) in &s.points {
...
fn auto_y_range(series: &[LineSeries]) -> (f64, f64) {
    let mut min = f64::MAX;
    let mut max = f64::MIN;
    for s in series {
        for (_, y) in &s.points {
